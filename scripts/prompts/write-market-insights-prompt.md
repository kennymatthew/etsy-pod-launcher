...
- Section 6 (Demand Signals Summary) is **fully auto-generated** by `generate-competitor-report.py` — do NOT write any interpretation lines there. There are no `[FILL IN]` slots in Section 6 anymore.
...
## Section 6 — Demand Signals Summary (fully auto-generated — do not write anything here)

This entire section is generated by `generate-competitor-report.py` every time the report runs. It includes:
- Verdict sentence (entrenchment-aware, auto-computed)
- Bestseller badge holder entrenchment table (entrenched / mid / accessible)
- Etsy Demand Labels table (Bestseller, Etsy's Pick, In-carts, N bought in 24h only — Rare Find and In High Demand removed)
- In-Carts Heat table
- Demand by Pattern Segment table with Entry Score column
- Pattern commentary (best and worst entry pattern, auto-named)

**Do not write any content in Section 6.** Just leave the heading `## 6. Demand Signals Summary` as a placeholder — the script will replace everything below it up to `## 7.`
